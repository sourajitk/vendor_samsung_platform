...
static int tune_sb_features(struct f2fs_sb_info *sbi)
{
	...
	/* Check default mount opts "flush_group=5666,inlinecrypt" */
	if (strncmp((char *)(sb->mount_opts), "flush_group=5666,inlinecrypt", 29)) {
		memset((char *)(sb->mount_opts), 0, sizeof(sb->mount_opts));
		strncpy((char *)(sb->mount_opts), "flush_group=5666,inlinecrypt", sizeof(sb->mount_opts));
		sb_changed = 1;
	}
